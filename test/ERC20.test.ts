...
import { BrillionToken, TestProxy } from "../typechain-types";
...
describe("ERC20", () => {
  ...
  let tokenImplementation: BrillionToken;

  before(async () => {
    [initialHolder, recipient, anotherAccount, proxyAdmin] = await ethers.getSigners();
    Token = await ethers.getContractFactory("BrillionToken");
  ...
  beforeEach(async () => {
    ...
    token = await ethers.getContractAt("BrillionToken", testProxy.address) as BrillionToken;
